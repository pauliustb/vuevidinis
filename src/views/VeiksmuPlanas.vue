            ...
            div(v-else)
              div(v-if="this.$store.getters['user/user'].role === 'frontuser'")
                div.add-plan(v-if="contactedMentors && contactedMentors.length > 0")
                  div
                  BtnSimple(v-bind:href="'#'" :wider="true" @click="creatingNewPlan = !creatingNewPlan" v-bind:active="false" color="blue" v-bind:disabled="disableAll" v-bind:loading="loadingBtn" v-bind:uppercase="false" v-bind:text="$t('planas.pridetiplana')" textAligin="center")
                    transition(name="fade")
                      div.new-plan(v-if="creatingNewPlan")
                        div.mb-2.select-mentor
                          h3.form-heading {{$t('planas.pasirinkitementoriu')}}
                          v-select(:options="contactedMentors"
                          :get-option-label="(contactedMentors) => contactedMentors.fullName"
                        ...
                        div.add-item.mt-2(v-if="selectedMentor")
                          BtnSimple(v-bind:href="'#'" @click="addPlanItem" v-bind:active="false" color="blue" v-bind:disabled="disableAll" v-bind:loading="loadingBtn" v-bind:uppercase="false" v-bind:text="$t('planas.pridetieilute')" textAligin="center")
                        div.add-row.mt-2(v-if="selectedMentor")
                          div.update-status
                            ...
                            div.success(v-if="updateSuccess")
                              ...
                              BtnSimple(v-bind:href="'#'" @click="savePlan" v-bind:active="false" :wider="true" color="darkblue" v-bind:disabled="savePlanSuccess" v-bind:loading="loadingSaveBtn" v-bind:uppercase="false" v-bind:text="$t('planas.issaugotiirsiustimentoriui')" textAligin="center")
                div(v-else)
